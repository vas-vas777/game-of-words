...
public:
	...
	std::multimap<size_t, std::wstring> words; // хранение слов опеределенной длины и начинающихс€ на конкретную букву
	std::map<std::wstring, int> letters_of_word; // буквы заданного слова, контейнер не содержит повтор€ющиес€ буквы
	std::vector<wchar_t> word;// дл€ хранени€ заданного слова
	bool LoadDictonary(char* Filename);
	void GuessWord(std::wstring str, size_t length);
};


bool Trie::LoadDictonary(char* Filename) //загрузка словар€
{
	...
	{
		wif >> Buff;
		if (Buff.find_first_of(L"…÷” ≈Ќ√Ўў«’Џ‘џ¬јѕ–ќЋƒ∆Ёя„—ћ»“№Ѕё") != std::wstring::npos) // если есть заглавные буквы
		{
			continue;
		}
		if (Buff.find_first_of(L"~`!@#$%^&*()_+=-?><.,\\|/;:'""єqwertyuiopasdfghjklzxcvbnmQWERTYUIOPASDFGHJKLZXCVBNM1234567890") != std::wstring::npos)
		{
			continue;
		}
...
int main(int argc, char *argv[])
{
	...
	if (Word.find_first_of(L"…÷” ≈Ќ√Ўў«’Џ‘џ¬јѕ–ќЋƒ∆Ёя„—ћ»“№Ѕё") != std::wstring::npos) // если есть заглавные буквы
	{
		out << L"error" << std::endl;
		return -1;
	}
	if (Word.find_first_of(L"~`!@#$%^&*()_+=-?><.,\\|/;:'""єqwertyuiopasdfghjklzxcvbnmQWERTYUIOPASDFGHJKLZXCVBNM1234567890") != std::wstring::npos)
	{
		out << L"error" << std::endl;
		return -1;
	}
